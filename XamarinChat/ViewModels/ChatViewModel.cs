...
namespace XamarinChat
{
	public class ChatViewModel : BaseViewModel
	{
		...
		void _chatServices_OnMessageReceived (object sender, ChatMessage e)
		{
            if (t_name != e.Name)
            {
                _messages.Add(new ChatMessageViewModel { Name = e.Name, Message = e.Message, IsMine = false });
            }
            else
            {
                _messages.Add(new ChatMessageViewModel { Name = e.Name, Message = e.Message, IsMine = true }); 
            }
